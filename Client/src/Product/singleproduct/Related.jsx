  ...
  const shoesdata = data.map((item) => {
    ...
    return (
      ...
      <div key={item._id} className='card-1 m-3'>
        <Link to={`/singlepro/${_id}`} style={{textDecoration:"none"}}> <div className='img-wrapper-2'>
          <img src={item.image} className="img-fluid inner-img-2 img-img" />
        </div>
          <div className="card-body mar-2">
            <h5 className="card-title" style={{color:"black"}}>category:{item.category}</h5>
            <p className="card-text" style={{color:"black"}}>price:{item.price}</p>
          </div>
        </Link>
